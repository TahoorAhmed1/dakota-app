  ...
  return (
    <div className="relative h-80 w-full overflow-hidden rounded-xl bg-[#d6ebe2] sm:h-105 lg:h-135">
      ...
      <div className="absolute bottom-3 right-3 flex flex-col overflow-hidden rounded border border-[#d9d9d9] bg-white shadow-sm sm:bottom-4 sm:right-4">
        <span className="border-b border-[#d9d9d9] px-3 py-2 text-lg text-[#777]">
          +
        </span>
...
function PropertyMap({ selectedId, onSelectCamp }: MapProps) {
  ...
  return (
    <div
      ref={mapWrapRef}
      className="relative h-80 w-full overflow-hidden rounded-xl sm:h-105 lg:h-135"
    >
      <div className="absolute left-3 top-3 z-1000 flex overflow-hidden rounded border border-[#cfcfcf] bg-white text-xs font-semibold text-[#444] shadow-sm sm:left-4 sm:top-4">
        <button
          ...
          className={`border-r border-[#d9d9d9] px-3 py-2 transition-colors sm:px-4 ${viewMode === "map" ? "bg-white text-[#3d3d3d]" : "bg-[#f6f6f6] text-[#7c7c7c]"}`}
        >
          Map
        </button>
        <button
          ...
          className={`px-3 py-2 transition-colors sm:px-4 ${viewMode === "satellite" ? "bg-white text-[#3d3d3d]" : "bg-[#f6f6f6] text-[#7c7c7c]"}`}
        >
          Satellite
        </button>
      </div>
      <button
        ...
        className="absolute right-14 top-3 z-1000 rounded border border-[#d9d9d9] bg-white px-3 py-2 text-[#777] shadow-sm transition hover:bg-[#fafafa] sm:right-16 sm:top-4"
      >
        &#9974;
      </button>
      <div className="absolute right-3 top-3 z-1000 flex flex-col overflow-hidden rounded border border-[#d9d9d9] bg-white shadow-sm sm:right-4 sm:top-4">
        <button
          ...
          onClick={zoomIn}
          aria-label="Zoom in"
          className="border-b border-[#d9d9d9] px-3 py-1.5 text-lg text-[#777] transition hover:bg-[#fafafa]"
        >
          +
        </button>
        <button
          ...
          onClick={zoomOut}
          aria-label="Zoom out"
          className="px-3 py-1.5 text-lg text-[#777] transition hover:bg-[#fafafa]"
        >
          -
        </button>
      </div>
...
function CampCard({
  ...
  return (
    <div
      ...
      className={`group cursor-pointer rounded-xl border bg-white p-5 transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-400 sm:p-6 ${isSelected ? "border-orange-400 shadow-lg shadow-orange-100" : "border-[#e0d3c4] shadow-sm hover:border-orange-300 hover:shadow-md"}`}
    >
      <div className="flex items-start gap-4">
        <span
          className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg text-sm font-bold transition-colors ${isSelected ? "bg-orange-500 text-white" : "bg-[#f5ece0] text-[#e67b35] group-hover:bg-orange-100"}`}
        >
          {camp.number}
        </span>
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-start justify-between gap-x-3 gap-y-1">
            <h3 className="text-sm font-bold leading-snug text-[#281703] sm:text-base">
              {camp.name}
            </h3>
            <span className="shrink-0 rounded-full bg-[#f0e8de] px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-[#a0724d]">
              {camp.region}
            </span>
          </div>
          <p className="mt-0.5 text-xs text-[#281703]/55">
            {camp.location} &middot; {camp.county}
          </p>
            ...
            <nav aria-label="Breadcrumb" className="mt-5 sm:mt-6">
              <ol className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#281703] sm:gap-3 sm:text-[11px]">
                <li>
                  <Link
                    href="/"
                    className="inline-flex items-center gap-2 transition-colors hover:text-[#e67b35]"
                  >
                    <svg
                      aria-hidden="true"
                ...
                <li aria-hidden="true" className="text-[#6c5240]">
                  &#8250;
                </li>
          ...
          <div className="mx-auto max-w-7xl">
            <div className="mb-10 text-center sm:mb-12">
              <p className="mb-2 text-xs font-bold tracking-widest uppercase text-orange-500">
                5 Properties &middot; South Dakota
              </p>
              <h2 className="text-3xl font-bold uppercase text-[#281703] sm:text-4xl">
                Pheasant Camp Locations
              </h2>
              <p className="mx-auto mt-3 max-w-2xl text-sm leading-relaxed text-[#281703]/70 sm:text-base">
                Select a camp card to fly the map to that location, or click a
                map pin to highlight the camp. Each property has unique habitat,
                lodging, and group capacity.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-8 lg:flex-row lg:items-start">
              <div className="w-full lg:sticky lg:top-24 lg:w-7/12">
                <PropertyMap
                  ...
                  onSelectCamp={toggleSelected}
                />
                <p className="mt-3 text-center text-xs text-[#281703]/45">
                  {selectedId
                    ? `Showing: ${camps.find((c) => c.id === selectedId)?.name}`
                    : "Click a pin or camp card to explore"}
                </p>
              </div>
              ...
              <div className="w-full space-y-4 lg:w-5/12">
                {camps.map((camp) => (
                  <CampCard
                    key={camp.id}
                    camp={camp}
                    isSelected={selectedId === camp.id}
                    onSelect={() => toggleSelected(camp.id)}
                  />
                ))}
